...
def split_text(path_input: str, encoding: str = None) -> pd.DataFrame:
    ...
    text_df.columns = ["fragment_{:02d}".format(
        i) for i in range(1, len(text_df.columns) + 1)]
    return text_df


def split_fragments_into_files(fragments: pd.DataFrame, path_output: str, encoding: str = None, verbose=False) -> None:
    """Guarda los fragmentos en archivos de texto en la ruta especificada."""
    crear_directorio(path_output, verbose=verbose)
    fragment_series = fragments.apply(
...
def convert_fragments_into_corpus(fragments: pd.DataFrame, path_output: str, encoding: str = None) -> pd.DataFrame:
    """
